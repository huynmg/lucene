...
public class TestRerankKnnFloatVectorQuery extends LuceneTestCase {
    ...
    public void testTwoPhaseKnnVectorQuery() throws Exception {
        ...
        try (IndexWriter writer = new IndexWriter(directory, config)) {
            for (int j = 0; j < numSegments; j++) {
                for (int i = 0; i < numVectors; i++) {
                    ...
                    doc.add(new KnnFloatVectorField("vector_2", vector, VECTOR_SIMILARITY_FUNCTION));
        ...
        try (IndexReader reader = DirectoryReader.open(directory)) {
            ...
            RerankFloatVectorQuery query = new RerankFloatVectorQuery(knnQuery, "vector_2", targetVector, k);
