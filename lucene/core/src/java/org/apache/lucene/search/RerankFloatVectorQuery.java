...
//            }
            ...
            DocIdSetIterator iterator = scorer.iterator();
            VectorScorer rerankScorer = floatVectorValues.scorer(target);
            DocIdSetIterator rerankIterator = rerankScorer.iterator();
            while (iterator.nextDoc() != DocIdSetIterator.NO_MORE_DOCS) {
                int docId = iterator.docID();
                rerankIterator.advance(docId);
                float score = rerankScorer.score();
                queue.insertWithOverflow(new ScoreDoc(leaf.docBase + docId, score));
            }
        }
        int i = 0;
